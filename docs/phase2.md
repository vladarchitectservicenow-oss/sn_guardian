# Phase 2: Build & Validate — Guardian

## Scope
This document covers the implementation, testing, quality assurance, and packaging phase for sn_guardian.
...
- **Repositories:** GitHub — `vladarchitectservicenow-oss/sn_guardian`.
...
- Script Include (`sn_guardianProcessor`) encapsulating business rules.
...
- Watermark updates in `x_sn_guardian_delta_watermark`.
- Report artifacts stored as attachments on `x_sn_guardian_report`.
...
- Scripted REST API (`sn_guardianAPI`) with named resources.
...
- Job status tracked in `x_sn_guardian_scan_log`.
